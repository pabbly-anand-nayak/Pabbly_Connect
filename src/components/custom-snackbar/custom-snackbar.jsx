...
export const SnackbarProvider = ({ children, sx }) => {
  ...
  // Wrap openSnackbar in useCallback to prevent re-creation
  const openSnackbar = useCallback(({ message, severity = 'info', hideDuration = 3000 }) => {
    setSnackbarState({ open: true, message, severity, hideDuration });
  }, []);
  ...
  return (
    <SnackbarContext.Provider value={value}>
      ...
      <Portal>
        <Snackbar
          open={snackbarState.open}
          autoHideDuration={snackbarState.hideDuration}
          onClose={closeSnackbar}
