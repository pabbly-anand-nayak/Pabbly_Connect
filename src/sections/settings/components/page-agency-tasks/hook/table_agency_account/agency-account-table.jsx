...
export default function AgencyAccountTable({
  ...
  useEffect(() => {
    const calculateTotalTasksAssigned = () => {
      console.log('Table Data:', tableData); // Log the entire table data

      const totalTasks = tableData.reduce((acc, row) => {
        const taskCount = Number(row.totalAmount) || 10000;
        console.log(
          `Row ID: ${row.id}, Total Amount: ${row.totalAmount}, Parsed Task Count: ${taskCount}`
        ); // Log each row's totalAmount

        return acc + taskCount;
      }, 0);

      console.log('Calculated Total Tasks Assigned:', totalTasks); // Log the final total
